...
class TagCloud {
  ...
  _createTextItem (text, index = 0) {
    ...
    // set properties
    itemEl.dataset.id = text.id || text.text
    ...
    itemEl.innerText = text.text || '未知标签'
    return {
      el: itemEl,
...
export default (els, data, options) => {
  if (typeof els === 'string') els = document.querySelectorAll(els)
  if (!els.forEach) els = [els]
  const instances = []
  els.forEach(el => {
    if (el) {
      instances.push(new TagCloud(el, data, options))
    }
  })
